...
class GraphWin(tk.Canvas):
    ...
    def __init__(self, title: str ="Graphics Window",
        ...
        master.lift()
        self.lastKey = ""
        # in case using dark mode on a Mac
    ...
    def checkKey(self) -> Optional[str]:
    ...
    def toScreen(self, x, y) -> Tuple[float, float]:
        trans = self.trans
        if trans:
    ...
    def toWorld(self, x: float, y: float) -> Tuple[float, float]:
        trans = self.trans
        if trans:
    ...
    def savePostscript(self, filename: str):
        """
        saves the window contents to a PostScript file
        :param filename: name of postscript file to create
        :return: None
        """
...
class _BBox(GraphicsObject):
    ...
    def __init__(self, p1: Point, p2: Point, options=None):
        if options is None:
